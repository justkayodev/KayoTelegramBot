...
def handle_update():
    ...
    if request.method == 'POST' and is_telegram_request(request.remote_addr or request.headers.get("X-Forwarded-For", "")):
        try:
            ...
            # If update is for a new user
            elif "message" in data and "new_chat_participant" in data["message"]:
                # logging.info("Calling method to send a welcome message.")
                welcome_msg = os.getenv("WELCOME_MSG")
                new_member = data["message"]["new_chat_participant"]

                new_member_name = new_member.get("first_name") or new_member.get("username") or new_member.get("last_name")
                welcome_msg = welcome_msg.format(new_member_name.capitalize())
...
def send_message(bot_token, channel_id, welcome_msg):
    ...
    try:
        ...
        logging.info("Response from send welcome message api call - {}".format(response.json()))
    except Exception as e:
        logging.error("Unable to send welcome message to the chat. Error - {}".format(e))
        resp["Status"] = "Failure"
